...
# thinking budget 上限（参考 kiro.rs-fork: 128000 for 1M context）
...
def _get_budget_from_suffix(model_lower: str) -> int:
    """从模型名后缀解析 thinking budget tokens
    
    支持粒度后缀（参考 kiro.rs-fork）：
    ...
    - -thinking         → 20000 (默认)
    """
    if model_lower.endswith("-thinking-minimal"):
        return 512
    elif model_lower.endswith("-thinking-low"):
        return 1024
    elif model_lower.endswith("-thinking-medium"):
        return 8192
    elif model_lower.endswith("-thinking-high"):
        return 24576
    elif model_lower.endswith("-thinking-xhigh"):
        return 32768
...
def parse_thinking_from_model_name(model: str) -> Optional[ThinkingConfig]:
    ...
    例如：
    - claude-sonnet-4-5-thinking        -> enabled, budget=20000
    - claude-sonnet-4-6-thinking        -> enabled, budget=20000
    - claude-opus-4-6-thinking          -> adaptive, effort=high
    - claude-opus-4-6-thinking-xhigh    -> adaptive, effort=high, budget=32768
    - claude-sonnet-4-5-thinking-medium -> enabled, budget=8192
    """
    model_lower = model.lower()
    if "thinking" not in model_lower:
        return None
    
    # Opus 4.6 用 adaptive 模式，effort=high（参考 kiro.rs-fork）
    ...
    budget_tokens = _get_budget_from_suffix(model_lower)
    
    return ThinkingConfig(
        enabled=True,
        thinking_type=thinking_type,
        budget_tokens=budget_tokens,
        effort="high",  # 始终 high（adaptive 模式下有效）
    )
...
def strip_thinking_suffix_from_model(model: str) -> str:
    """去掉模型名中的 -thinking 后缀（含粒度后缀），得到实际模型 ID
    
    例如：
    - claude-sonnet-4-5-thinking        -> claude-sonnet-4-5
    - claude-opus-4-6-thinking-xhigh    -> claude-opus-4-6
    """
    return re.sub(r'-thinking(?:-(?:minimal|low|medium|high|xhigh))?$', '', model, flags=re.IGNORECASE)
...
class ThinkingStreamParser:
    ...
    def flush_pending_text_for_tool_use(self) -> list:
        """在 tool_use 边界前冲刷暂存文本，避免文本被工具块打断。

        参考 kiro.rs 的处理：thinking 模式下，解析器可能为了标签探测
        暂存一小段尾部文本；当即将进入 tool_use 块时应先发出这些文本。
        """
        if not self.thinking_enabled:
            return []
        if self.in_thinking_block:
            return []
        if not self.thinking_extracted:
            return []
        if not self.buffer:
            return []
        ...
        self.buffer = ""
        return [{"type": "text", "text": buffered}]
