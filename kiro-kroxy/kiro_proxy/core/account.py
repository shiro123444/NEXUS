...
class Account:
    ...
    def mark_quota_exceeded(self, reason: str = "Rate limited"):
        ...
        - 普通限流：使用 rate_limiter 的冷却时间
        - 月度配额耗尽：使用更长冷却，避免请求在耗尽账号上循环重试
        """
        from .rate_limiter import get_rate_limiter
        rate_limiter = get_rate_limiter()
        reason_lower = (reason or "").lower()
        is_monthly_exhausted = "monthly_request_count" in reason_lower or "monthly quota" in reason_lower

        cooldown = rate_limiter.get_quota_cooldown_seconds()
        if is_monthly_exhausted:
            try:
                cooldown = int(os.getenv("KIRO_MONTHLY_QUOTA_COOLDOWN_SECONDS", "21600"))
            except Exception:
                cooldown = 21600
            cooldown = max(cooldown, 300)
        
        # 月度耗尽必须冷却；普通限流仅在限速启用时冷却
        if is_monthly_exhausted or rate_limiter.should_apply_quota_cooldown():
