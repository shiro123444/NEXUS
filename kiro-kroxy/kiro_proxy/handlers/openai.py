...
def _apply_sonnet46_fallback_to_request(kiro_request: dict, fallback_model: str = "claude-sonnet-4.5") -> None:
    ...
    for h_msg in conv.get("history", []) or []:
        if isinstance(h_msg, dict) and "userInputMessage" in h_msg and isinstance(h_msg["userInputMessage"], dict):
...
async def handle_chat_completions(request: Request):
    ...
    if request_cache_key:
        print(f"[OpenAI] Using SDK cache/session key for sticky routing: {request_cache_key[:64]}")
    ...
    if stream:
        return await _handle_openai_stream(
            ...
            images, tool_results, thinking_config
        )
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                if (
                    ...
                    print(f"[OpenAI] claude-sonnet-4.6 容量不足，自动降级到 {fallback_model}")
...
async def _handle_openai_stream(
    ...
    kiro_tools: list, images: list, tool_results: list,
                ...
                async with http_pool.api_client.stream("POST", KIRO_API_URL, json=kiro_request, headers=req_headers, timeout=60.0) as resp:
                    ...
                    if resp.status_code != 200:
                        ...
                        if resp.status_code == 429:
                            if (
                                _is_sonnet46_fallback_enabled()
                                and "INSUFFICIENT_MODEL_CAPACITY" in error_text
                                and "sonnet-4.6" in model
                            ):
                                fallback_model = "claude-sonnet-4.5"
                                print(f"[OpenAI Stream] claude-sonnet-4.6 容量不足，自动降级到 {fallback_model}")
                                _apply_sonnet46_fallback_to_request(kiro_request, fallback_model)
                                continue
                            current_account.error_count += 1
                            if retry < max_retries:
