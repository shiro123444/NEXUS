...
from ..config import KIRO_API_URL, map_model_name, is_disallowed_gateway_model
...
from ..converters import extract_request_cache_key, build_session_id_with_cache_key
...
def _apply_sonnet46_fallback_to_request(kiro_request: dict, fallback_model: str = "claude-sonnet-4.5") -> None:
    """Update current message and history modelId in-place for fallback retry."""
    conv = kiro_request.get("conversationState", {}) if isinstance(kiro_request, dict) else {}
    current = conv.get("currentMessage", {}) if isinstance(conv, dict) else {}
    user_msg = current.get("userInputMessage", {}) if isinstance(current, dict) else {}
    if isinstance(user_msg, dict):
        user_msg["modelId"] = fallback_model
    for history_msg in conv.get("history", []) or []:
        if isinstance(history_msg, dict) and isinstance(history_msg.get("userInputMessage"), dict):
            history_msg["userInputMessage"]["modelId"] = fallback_model
...
def _convert_responses_input_to_kiro(input_data, instructions: str = None):
    ...
    for i, item in enumerate(input_data):
        ...
        # Compatibility: some clients (e.g. CherryStudio) send chat-style entries
        # with role/content but without an explicit type.
        ...
        if item_type == "message":
            ...
            if isinstance(content_list, str):
                content_list = [{"type": "input_text", "text": content_list}]
            ...
            if role == "system":
                # Preserve system prompt semantics by prepending to instructions.
                sys_text = text.strip()
                if sys_text:
                    if instructions:
...
async def handle_responses(request: Request):
    ...
    debug_file = f"{debug_dir}/{log_id}_original_request.json"
    with open(debug_file, 'w', encoding='utf-8') as f:
        json.dump(body, f, indent=2, ensure_ascii=False)
    print(f"[Responses] Saved original request to {debug_file}")
    print(f"[Responses] Request keys: {list(body.keys())}")
    ...
    default_session_id = hashlib.sha256(session_str.encode()).hexdigest()[:16]
    ...
    if request_cache_key:
        print(f"[Responses] Using SDK cache/session key: {request_cache_key[:64]}")
    ...
    # 非流式
    sonnet46_fallback_attempted = False
    while True:
        resp = await http_pool.api_client.post(KIRO_API_URL, json=kiro_request, headers=headers)
        if (
            not sonnet46_fallback_attempted
            and resp.status_code == 429
            and _is_sonnet46_fallback_enabled()
            and "INSUFFICIENT_MODEL_CAPACITY" in resp.text
            and "sonnet-4.6" in model
        ):
            fallback_model = "claude-sonnet-4.5"
            print(f"[Responses] claude-sonnet-4.6 容量不足，自动降级到 {fallback_model}")
            _apply_sonnet46_fallback_to_request(kiro_request, fallback_model)
            sonnet46_fallback_attempted = True
            continue
        break
    if resp.status_code != 200:
...
def _build_response(result: dict, model: str, response_id: str) -> dict:
    ...
    cached_tokens = int(result.get("cache_read_input_tokens", 0) or 0)
...
async def _handle_stream(kiro_request, headers, account, model, log_id, start_time, thinking_config):
    ...
    async def generate():
                ...
                async with http_pool.api_client.stream("POST", KIRO_API_URL, json=kiro_request, headers=headers) as response:
                    ...
                    if response.status_code != 200:
                        ...
                        if (
                            ...
                            and _is_sonnet46_fallback_enabled()
                            and "INSUFFICIENT_MODEL_CAPACITY" in error_msg
                            and "sonnet-4.6" in model
                        ):
                            fallback_model = "claude-sonnet-4.5"
                            print(f"[Responses] claude-sonnet-4.6 容量不足，自动降级到 {fallback_model}")
                    ...
                    while True:
                        ...
                        # Keep a copy of upstream raw event-stream payload so we can recover usage tokens.
                        raw_stream_bytes.extend(chunk)
        ...
        # Recover token usage from upstream event-stream payload.
        input_tokens = 0
        output_tokens = 0
        cached_tokens = 0
        ...
        if raw_stream_bytes:
            try:
                parsed = parse_event_stream_full(bytes(raw_stream_bytes))
                input_tokens = int(parsed.get("input_tokens", 0) or 0)
                output_tokens = int(parsed.get("output_tokens", 0) or 0)
                cached_tokens = int(parsed.get("cache_read_input_tokens", 0) or 0)
                cache_creation_tokens = int(parsed.get("cache_creation_input_tokens", 0) or 0)
            except Exception as exc:
                print(f"[Responses] Usage parse failed: {exc}")
