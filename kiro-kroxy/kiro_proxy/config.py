...
# 网关禁用模型关键字（不再支持 GPT/Codex）
DISALLOWED_MODEL_TOKENS = ("gpt", "codex", "chatgpt")
...
MODEL_MAPPING = {
    ...
    "claude-sonnet-4-6-20250218": "claude-sonnet-4.6",
    # 别名
...
# 内置 Kiro 模型
BUILTIN_KIRO_MODELS = {"auto", "claude-sonnet-4.6", "claude-sonnet-4.5", "claude-sonnet-4", "claude-haiku-4.5", "claude-opus-4.5", "claude-opus-4.6"}

# Agentic 模型变体（带 -agentic 后缀，映射到基础模型 + 注入 agentic 系统提示）
AGENTIC_MODEL_VARIANTS = {
    "claude-sonnet-4-5-agentic", "claude-sonnet-4.5-agentic",
    "claude-opus-4-5-agentic", "claude-opus-4.5-agentic",
    "claude-opus-4-6-agentic", "claude-opus-4.6-agentic",
...
THINKING_MODEL_VARIANTS = {
    ...
    # 粒度后缀变体（-thinking-minimal/low/medium/high/xhigh）
    "claude-sonnet-4-6-thinking-minimal", "claude-sonnet-4.6-thinking-minimal",
    "claude-sonnet-4-6-thinking-low",     "claude-sonnet-4.6-thinking-low",
    "claude-sonnet-4-6-thinking-medium",  "claude-sonnet-4.6-thinking-medium",
    "claude-sonnet-4-6-thinking-high",    "claude-sonnet-4.6-thinking-high",
    "claude-sonnet-4-6-thinking-xhigh",   "claude-sonnet-4.6-thinking-xhigh",
    "claude-opus-4-6-thinking-minimal",   "claude-opus-4.6-thinking-minimal",
    "claude-opus-4-6-thinking-low",       "claude-opus-4.6-thinking-low",
    ...
    "claude-opus-4-6-thinking-xhigh",     "claude-opus-4.6-thinking-xhigh",
    "claude-sonnet-4-5-thinking-minimal", "claude-sonnet-4.5-thinking-minimal",
    "claude-sonnet-4-5-thinking-low",     "claude-sonnet-4.5-thinking-low",
    "claude-sonnet-4-5-thinking-medium",  "claude-sonnet-4.5-thinking-medium",
    "claude-sonnet-4-5-thinking-high",    "claude-sonnet-4.5-thinking-high",
    "claude-sonnet-4-5-thinking-xhigh",   "claude-sonnet-4.5-thinking-xhigh",
}
...
def is_disallowed_gateway_model(model: str) -> bool:
    """检查模型名是否属于网关禁用族（GPT/Codex）。"""
    model_lower = (model or "").strip().lower()
    if not model_lower:
        return False
    return any(token in model_lower for token in DISALLOWED_MODEL_TOKENS)

def get_context_window_size(model: str) -> int:
    """根据模型名返回上下文窗口大小

    与 kiro.rs 对齐：Sonnet/Opus 4.6 为 1,000,000，其余为 200,000。
    """
...
def map_model_name(model: str) -> str:
    ...
    - Gemini 别名: gemini-1.5-pro → claude-sonnet-4.5
    ...
    # 支持 agentic 变体：保留原始名称（handler 层会解析 -agentic 后缀）
    if model in AGENTIC_MODEL_VARIANTS:
        return model
    normalized_for_agentic = _normalize_model_id(model)
    if normalized_for_agentic in AGENTIC_MODEL_VARIANTS:
        return normalized_for_agentic
    
    # 支持 thinking 变体（含粒度后缀）：保留原始名称（handler 层会解析 -thinking 后缀）
    ...
    # 粒度后缀 (-thinking-minimal/low/medium/high/xhigh) 动态支持
    _thinking_granularity_re = re.compile(
        r'^(.+)-thinking-(minimal|low|medium|high|xhigh)$', re.IGNORECASE
    )
    if _thinking_granularity_re.match(model):
        return model  # handler 层解析
    normalized_gran = _normalize_model_id(model)
    if _thinking_granularity_re.match(normalized_gran):
        return normalized_gran
