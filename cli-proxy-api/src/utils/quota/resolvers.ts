...
export function extractCodexChatgptAccountId(value: unknown): string | null {
  ...
  const direct = normalizeStringValue(payload.chatgpt_account_id ?? payload.chatgptAccountId);
  ...
  const nested = normalizeStringValue(
    authClaims?.chatgpt_account_id ?? authClaims?.chatgptAccountId
  );
  if (nested) return nested;

  return normalizeStringValue(payload.account_id ?? payload.accountId);
}
...
export function resolveCodexChatgptAccountId(file: AuthFileItem): string | null {
  ...
  const tokenCandidates = [file.access_token, metadata?.access_token, attributes?.access_token];
