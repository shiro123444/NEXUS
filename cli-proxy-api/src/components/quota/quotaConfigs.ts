...
>();
const codexAccountIdCache = new Map<string, string>();
...
const fetchCodexQuota = async (
  ...
  let accountId = codexAccountIdCache.get(authIndex) ?? resolveCodexChatgptAccountId(file);

  if (!accountId) {
    const fileName = normalizeStringValue(file.name ?? file.id);
    if (fileName) {
      try {
        const rawAuthFile = await authFilesApi.downloadJsonObject(fileName);
        const resolvedFromRaw = resolveCodexChatgptAccountId(rawAuthFile as AuthFileItem);
        if (resolvedFromRaw) {
          accountId = resolvedFromRaw;
        }

        if (!planTypeFromFile) {
          planTypeFromFile = resolveCodexPlanType(rawAuthFile as AuthFileItem);
        }
      } catch {
        // Keep existing error behavior when fallback loading fails.
      }
    }
  }

  if (accountId) {
    codexAccountIdCache.set(authIndex, accountId);
  }
